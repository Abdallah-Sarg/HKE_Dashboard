...
function getAuthenticatedDashboardUser(req) {
  ...
  return {
    ...
    global_name: String(session.global_name || ''),
...
async function fetchDiscordUserGuilds(accessToken) {
  ...
  if (!response.ok) {
    const responseText = await response.text().catch(() => '');
    throw new Error('discord_guilds_fetch_failed:' + (responseText || response.status));
  }
  return response.json();
}

async function discordBotApi(pathname) {
  ...
  if (!response.ok) {
    ...
    throw new Error('discord_bot_api_failed:' + response.status + ':' + text);
  }
  return response.json();
}

function parseGuildPermissionBits(value) {
  try {
...
function userGuildHasDashboardAccess(guildInfo) {
  const bits = parseGuildPermissionBits(guildInfo?.permissions);
  const ADMIN = 0x8n;
  const MANAGE_GUILD = 0x20n;
  return (bits & ADMIN) === ADMIN || (bits & MANAGE_GUILD) === MANAGE_GUILD;
}

function getAvailableCommandNames(client) {
  if (client?.commands?.size) {
    return Array.from(client.commands.keys())
  ...
  return fs.readdirSync(commandsDir)
    ...
    .map(file => {
      try {
        ...
        return String(mod?.name || '').trim().toLowerCase();
      } catch (_) {
        return '';
      }
    })
...
async function processDiscordAuthCallback(req, res) {
  ...
  try {
    ...
    const guilds = await fetchDiscordUserGuilds(tokenPayload.access_token).catch(() => []);
    setDashboardSession(res, req, {
      id: user.id,
...
async function getGuildDashboardData(client, guildId) {
  ...
  let guildInfo = guild ? { id: guild.id, name: guild.name, members: guild.memberCount } : null;
  ...
  if (guild) {
    ...
    try {
      const [guildMeta, guildRoles, guildChannels] = await Promise.all([
        discordBotApi(`/guilds/${guildId}`),
        discordBotApi(`/guilds/${guildId}/roles`),
        discordBotApi(`/guilds/${guildId}/channels`),
      ]);
      guildInfo = { id: String(guildMeta.id), name: String(guildMeta.name || guildId), members: Number(guildMeta.approximate_member_count || 0) };
      roles = Array.isArray(guildRoles)
        ? guildRoles
        ...
        : [];
      channels = Array.isArray(guildChannels)
        ? guildChannels
  ...
  const commands = getAvailableCommandNames(client);

  return {
    guild: guildInfo || { id: guildId, name: guildId, members: 0 },
    settings,
    autoReplies,
    roles,
    channels,
...
async function guildExistsForDashboard(client, guildId) {
  if (client?.guilds?.cache?.has(guildId)) return true;
  try {
    await discordBotApi(`/guilds/${guildId}`);
    return true;
  } catch (_) {
    return false;
  }
...
async function listAccessibleGuilds(client, userId) {
  const fallbackUser = typeof userId === 'object' && userId ? userId : null;
  const userGuilds = fallbackUser?.guilds || [];
  if (!client?.guilds?.cache?.size) {
    const out = [];
    for (const guildInfo of userGuilds) {
      if (!userGuildHasDashboardAccess(guildInfo)) continue;
      try {
        await discordBotApi(`/guilds/${guildInfo.id}`);
        out.push({ id: String(guildInfo.id), name: String(guildInfo.name || guildInfo.id), members: 0 });
      } catch (_) {
        // bot is not in guild or cannot access it
      }
    }
  ...
  for (const guild of client.guilds.cache.values()) {
    const member = await getDashboardMemberByUserId(guild, fallbackUser?.id || userId);
...
function createDashboardApp(client) {
  ...
  app.get('/api/guilds', (req, res) => {
    (async () => {
      const user = getAuthenticatedDashboardUser(req);
      if (!user) return res.status(401).json({ error: 'not_logged_in' });
      const guilds = await listAccessibleGuilds(client, user);
  ...
  app.use('/api/guilds/:guildId', async (req, res, next) => {
    ...
    const guildEntry = Array.isArray(user.guilds) ? user.guilds.find(g => String(g.id) === String(guildId)) : null;
    if (!guildEntry || !userGuildHasDashboardAccess(guildEntry)) return res.status(403).json({ error: 'forbidden_guild' });
    try {
      await discordBotApi(`/guilds/${guildId}`);
    } catch (_) {
  ...
  app.put('/api/guilds/:guildId/settings', async (req, res) => {
    const guildId = req.params.guildId;
    if (!await guildExistsForDashboard(client, guildId)) return res.status(404).json({ error: 'guild_not_found' });
    const p = req.body || {};
  ...
  app.post('/api/guilds/:guildId/command-aliases', async (req, res) => {
    ...
    const commands = getAvailableCommandNames(client);
    if (!commands.includes(n.command_name)) return res.status(400).json({ error: 'unknown_command' });

    const aliasConflicts = commands.includes(n.alias_name) && n.alias_name !== n.command_name;
    if (aliasConflicts) {
  ...
  app.locals.dashboardPort = port;
  return app;
}

function startDashboard(client) {
  const app = createDashboardApp(client);
  const startPort = app.locals.dashboardPort || parseIntSafe(process.env.PORT || process.env.DASHBOARD_PORT, 3000);
